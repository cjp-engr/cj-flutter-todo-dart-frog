...
class TodoControllerImpl implements TodoController {
  ...
  Future<Response> createTodo(RequestContext context) async {
    ...
    if (title == null || title.trim().isEmpty) {
      return Response.json(
        statusCode: HttpStatus.badRequest,
        body: {'message': 'Please provide valid todo title'},
      );
    }
  ...
  Future<Response> fetchTodos(RequestContext context) async {
    ...
    final result = await _fetchTodosUsecase(
      FetchTodosRequestModel(userId: user.id, match: match, sort: sort),
    );
  ...
  @override
  Future<Response> fetchTodo(RequestContext context, String todoId) async {
    final user = context.read<UserResponseModel>();
    ...
    return result.match(
      ...
      (todo) {
        return Response.json(body: {'todo': todo});
      },
    );
  }

  @override
  Future<Response> toggleTodo(RequestContext context, String todoId) async {
    final user = context.read<UserResponseModel>();
  ...
  @override
  Future<Response> updateTodo(RequestContext context, String todoId) async {
    final body = await context.request.json() as MapData;
    final title = body['title'] as String?;
    final description = body['description'] as String?;
  ...
  @override
  Future<Response> deleteTodo(RequestContext context, String todoId) async {
    final user = context.read<UserResponseModel>();
